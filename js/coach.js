// ProRun Coach — Groq API (gratuito) con Llama 3.3 70B
// Cuenta gratis en console.groq.com — 14.400 req/día sin coste
...
export class CoachAI {
  ...
  buildContext(state) {
    const { config, activities, planGenerator } = state;
    const lines = [];
  ...
  async sendMessage(userMessage, context) {
    if (!this.hasApiKey()) {
      throw new Error('Configura tu Groq API Key en Ajustes. Es gratis en console.groq.com');
    }
    ...
    if (resp.status === 401) throw new Error('Groq API Key inválida. Verifica en Ajustes.');
